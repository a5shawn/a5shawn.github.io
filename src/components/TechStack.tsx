import {
  SiJavascript, SiTypescript, SiVuedotjs, SiReact, SiElectron,
  SiNodedotjs, SiDeno, SiMysql, SiRedis, SiMongodb,
  SiNginx, SiDocker, SiKubernetes,
} from 'react-icons/si'
...
const categories: TechCategory[] = [
  {
    ...
    items: [
      { name: 'JavaScript' }, { name: 'TypeScript' }, { name: 'Vue' },
      { name: 'React' }, { name: 'Electron' }, { name: 'Uniapp' },
    ],
  },
  {
    id: 'backend', name: '后端开发', icon: '⚡',
    items: [
  ...
  },
  {
    ...
    items: [
      { name: 'MySQL' }, { name: 'Redis' }, { name: 'MongoDB' },
    ],
  },
  {
    id: 'devops', name: '运维', icon: '⚙',
    items: [
      { name: 'Nginx' }, { name: 'Docker' }, { name: 'K8s' },
    ],
  },
]
    ...
    <div className="tech-panel" id="tech">
      ...
      <div className="tech-grid-2x2" style={{ marginTop: 8 }}>
        {categories.map((cat, i) => (
          <div key={cat.id} className={`tech-card fade-in`} style={{ animationDelay: `${0.15 + i * 0.08}s` }}>
            ...
            <div className="tech-card-items">
              {cat.items.map((item) => (
                <span key={item.name} className="tech-card-item">
                  {iconMap[item.name] && <span className="tech-item-icon">{iconMap[item.name]}</span>}
                  {item.name}
                  {item.detail && <span className="tech-item-detail"> ({item.detail})</span>}
                </span>
              ))}
            </div>
          </div>
